...
/* -------------admin_password_change------------*/
	...
	public function add_new_task($data){
		...
		$dinas  = $this->test_form_input_data($data['dinas']);
		...
		try{
			$add_task = $this->db->prepare("INSERT INTO task_info (t_title, dinas, t_description, t_start_time, t_end_time, t_user_id) VALUES (:x, :d, :y, :z, :a, :b) ");

			$add_task->bindparam(':x', $task_title);
			$add_task->bindparam(':y', $task_description);
			$add_task->bindparam(':d', $dinas);
		...
		public function update_task_info($data, $task_id, $user_role){
			...
			try{
				$update_task = $this->db->prepare("UPDATE task_info SET t_title = :x, t_description = :y, dinas = :d, t_start_time = :z, t_end_time = :a, t_user_id = :b, status = :c WHERE task_id = :id ");

				$update_task->bindparam(':x', $task_title);
				$update_task->bindparam(':y', $task_description);
				$update_task->bindparam(':d', $dinas);
